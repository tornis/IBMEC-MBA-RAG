"""
00_check_ambiente.py - Verifica se o ambiente da Aula 2 esta pronto.

O que ele testa (um item por linha, com OK ou FALHA):
  1. Versao do Python
  2. Bibliotecas principais (haystack, docling, langchain splitters, ollama, etc.)
  3. Arquivo .env e chaves esperadas
  4. Datasets da Aula 2 (PDFs e corpus de exemplo)
  5. OpenSearch respondendo + plugin kNN
  6. Ollama respondendo + modelo de embedding baixado
  7. Chave da Groq presente (e, opcionalmente, um teste real de chamada)
...
def checar_python():
    ...
    if v >= (3, 10):
        ...
        falha(f"Python {v.major}.{v.minor} - o curso pede 3.10 ou superior")
...
def checar_datasets():
    print("4) Datasets da Aula 2")
    if _comum.CORPUS_SAMPLE.exists():
        ok(f"corpus de exemplo: {_comum.CORPUS_SAMPLE.name}")
    else:
        aviso("corpus_juridico_sample.json nao encontrado")
    for pdf in [_comum.PDF_DIGITAL, _comum.PDF_ESCANEADO]:
        if pdf.exists():
            ok(f"PDF: {pdf.name}")
        else:
            aviso(f"PDF nao encontrado: {pdf.name}")
...
def checar_opensearch():
    ...
    try:
        ...
        aviso("Veja o GUIA_OPENSEARCH_WINDOWS.md da Aula 1 para subir o servidor")
...
def main():
    parser = argparse.ArgumentParser(description="Verifica o ambiente da Aula 2.")
    parser.add_argument("--testar-groq", action="store_true",
                        help="faz uma chamada real na Groq para validar a chave")
    args = parser.parse_args()

    print("=" * 60)
    print("  CHECAGEM DE AMBIENTE - MBA RAG & CAG - Aula 2")
    print("=" * 60)
    checar_python()
    checar_bibliotecas()
    checar_env()
