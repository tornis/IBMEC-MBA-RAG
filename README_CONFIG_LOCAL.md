# Guia para instalação das aplicações em ambiente local
...
> O presente guia é uma sugestão para utilização em ambiente corporativo com restrição ao uso de aplicações comerciais e poucos recursos de hardware. 
...
- **Recomendação operacional**: criar uma VM para cada aplicação (Ollama e vLLM) quando houver recursos.
- **Dimensionamento de GPU**: manter os modelos residentes na VRAM por períodos longos reduz recarga e latência (ex.: `ollama run <model_name> --keepalive -1`).
...
- **FastAPI na VM CPU**: aplicações FastAPI compartilhadas pelos usuários (serviços intermediários) podem ser instaladas em uma VM com CPU.
- **FastAPI do usuário**: aplicações individuais de teste para consumir modelos devem ser instaladas no ambiente do usuário.
- **Versões de referência validadas (maio/2026)**: vLLM 0.19.1, OpenSearch 3.6.0, Langfuse 3.174.1, Ollama 0.24.0, Langflow 1.9.2, FastAPI 0.136.1, Uvicorn 0.47.0, Docling 2.93.0 (é mandatório verificar as releases oficiais no momento da instalação).
...
| OpenSearch | Banco de dados | ✅ Sim — índices vetoriais e documentos | ⚠️ Depende do perfil (LAB sem login / produção com login) | Persiste todos os documentos e vetores indexados. Em LAB local pode rodar sem autenticação; em produção deve usar credenciais e TLS. |
...
> **Resumo prático**: as aplicações que exigem criação de conta/login no cenário padrão são **Langfuse** (conta web + chaves de projeto) e **Langflow** (quando autenticação habilitada). O **OpenSearch** pode operar sem login no perfil LAB local, mas deve usar autenticação em produção.
...
> [→ Configuração e instalação](#anexo-1-vllm)
...
> [→ Configuração e instalação](#anexo-2-ollama)
...
> [→ Configuração e instalação](#anexo-3-ragatouille)
...
> [→ Configuração e instalação](#anexo-4-llmlingua)
...
> [→ Configuração e instalação](#anexo-5-opensearch)
...
> [→ Configuração e instalação](#anexo-6-langfuse)
...
> [→ Configuração e instalação](#anexo-7-langflow)
...
> [→ Configuração e instalação](#anexo-8-fastapi)
...
> [→ Configuração e instalação](#anexo-9-tesseract)
...
> [→ Configuração e instalação](#anexo-10-docling)
...
# Anexos — Configuração e Instalação

## Valide as versões de cada aplicação antes.
```bash
...
```
## Sugestões de código
- Os códigos abaixo não foram testados em ambiente local.
- Recomendável verificar a documentação oficial de cada aplicação
- Caso os comandos apresentem algum erro, realizar a correção na documentação e enviar novo pull request para o projeto.

<a name="anexo-1-vllm"></a>
## Anexo 1 — vLLM
...
# Configurar runtime para Podman rootless (executar sem sudo para gravar no home do usuário)
...
# avaliar a exposição da porta 11434
...
```bash
# Requer NVIDIA Container Toolkit instalado (ver Anexo 1 — vLLM)
...
<a name="anexo-3-ragatouille"></a>
## Anexo 3 — RAGatouille / ColBERT
...
<a name="anexo-4-llmlingua"></a>
## Anexo 4 — LLMLingua

### Instalação no Ubuntu Server

```bash
# Compartilha venv com RAGatouille — ambos necessitam de PyTorch compatível
source ~/.venvs/ragatouille/bin/activate
...
# 5. Perfil LAB local sem SSL (desativa plugin de segurança para simplificar os labs)
sudo tee /etc/opensearch/opensearch.yml.d/lab-local.yml > /dev/null <<'EOF'
...
# 2.1. Criar rede dedicada para OpenSearch + Dashboards
...
```bash
# 3. Executar container em perfil LAB local sem SSL (flags `:z` para compatibilidade com SELinux e volumes mapeados)
...
# 4. Verificar (aguardar ~30s para inicializar a JVM)
curl http://localhost:9200

# 5. OpenSearch Dashboards (opcional — interface gráfica)
...
read -s -p "Senha inicial admin OpenSearch: " OPENSEARCH_ADMIN_PASSWORD
export OPENSEARCH_ADMIN_PASSWORD

# 4. Executar container com plugin de segurança habilitado (padrão)
podman run -d --name opensearch \
  --network opensearch-net \
  -p 9200:9200 \
  -p 9600:9600 \
...
# 5. Verificar endpoint HTTPS (certificado pode ser self-signed no bootstrap)
...
# Perfil LAB_LOCAL (recomendado para aulas presenciais)
...
OPENSEARCH_PASS=<senha definida em ambiente seguro>
```

---

<a name="anexo-6-langfuse"></a>
## Anexo 6 — Langfuse
...
podman run -d --name langflow \
  -p 7860:7860 \
  -v langflow-data:/data/langflow:z \
...
<a name="anexo-8-fastapi"></a>
## Anexo 8 — FastAPI + Uvicorn
...
<a name="anexo-9-tesseract"></a>
## Anexo 9 — Tesseract-OCR
...
> **Nota**: Tesseract é instalado diretamente no SO hospedeiro. Não há imagem de container recomendada para uso como biblioteca pelos notebooks. O Docling pode usar Tesseract quando configurado para esse backend; o EasyOCR não depende do binário do Tesseract.

---

<a name="anexo-10-docling"></a>
## Anexo 10 — Docling

### Instalação no Ubuntu Server

```bash
# Pré-requisito: Tesseract instalado no SO (ver Anexo 9)

# Instalar Docling com extras de OCR no escopo do projeto
source ~/.venvs/projeto/bin/activate
